...
// (parallel only with explicit ask) so that background invocations don't
// trample each other.
const claudeQueue = (() => {
...
function callClaude(messages) {
  return new Promise((resolve, reject) => {
    // Render the chat history as a single prompt — claude --print is
    // one-shot, so we re-include the conversation each time. Cheap; the
    // model context is small and Claude handles the role labels naturally.
